...
# # Get bucket names with account ID (same name_prefix as terraform: project-env or project-env-suffix)
# RESOURCE_SUFFIX=$(
#     grep -E '^[[:space:]]*resource_suffix[[:space:]]*=' terraform.tfvars 2>/dev/null |
#         head -1 |
#         sed -E 's/^[[:space:]]*resource_suffix[[:space:]]*=[[:space:]]*"([^"]*)".*/\1/'
# )
# NAME_PREFIX="${PROJECT_NAME}-${ENVIRONMENT}"
# if [ -n "$RESOURCE_SUFFIX" ]; then
#     NAME_PREFIX="${NAME_PREFIX}-${RESOURCE_SUFFIX}"
# fi
