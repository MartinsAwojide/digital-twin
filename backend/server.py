...
def call_openrouter(conversation: List[Dict], user_message: str) -> str:
    """OpenAI-compatible API via OpenRouter (fallback when Bedrock fails)."""
    api_key = os.getenv("OPENROUTER_API_KEY")
    if not api_key:
        raise HTTPException(
            status_code=503,
            detail="Bedrock failed and OPENROUTER_API_KEY is not set",
        )
    client = OpenAI(
        api_key=api_key,
...
def call_bedrock(conversation: List[Dict], user_message: str) -> str:
    """Call AWS Bedrock; on most Bedrock failures, fall back to OpenRouter."""
    ...
    try:
        ...
        print(f"Bedrock error ({error_code}), using OpenRouter fallback: {e}")
